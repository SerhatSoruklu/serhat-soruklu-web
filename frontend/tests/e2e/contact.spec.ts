...
async function fillValidContactForm(page: Page): Promise<void> {
  ...
  await page.locator('#contact-message').fill('I would like to discuss a systems architecture project with clear boundaries and practical delivery context.');
...
test.describe('contact page', () => {
  for (const viewport of viewports) {
    test(`submits successfully with responsive layout at ${viewport.name}`, async ({ page }, testInfo) => {
      ...
      const messageHeight = await page.locator('#contact-message').evaluate((element) => element.getBoundingClientRect().height);
      expect(messageHeight).toBeLessThanOrEqual(viewport.width >= 1280 ? 122 : 136);
      await expect(page.getByRole('button', { name: 'Send Message' })).toBeDisabled();

      if (viewport.width >= 1366) {
        const formTop = await page.locator('.contact-frame').evaluate((element) => element.getBoundingClientRect().top);
  ...
  test('blocks whitespace-only names and messages before the API request', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: 'Send Message' })).toBeDisabled();
    await expect(page.locator('#contact-first-name-feedback')).toContainText('First name is required.');
    await expect(page.locator('#contact-last-name-feedback')).toContainText('Last name is required.');
    await expect(page.locator('#contact-message-feedback')).toContainText('Message is required.');
    expect(requestCount).toBe(0);
  });

  test('rejects recipient lists and display-name email wrappers before submission', async ({ page }) => {
    await page.goto('/contact');
    await fillValidContactForm(page);
  ...
  test('renders backend validation failures in the custom error UI', async ({ page }, testInfo) => {
    ...
    await expect(page.locator('#contact-message-feedback')).toContainText('Message looks automated.');
    await expectNoHorizontalOverflow(page);
  });
  ...
  test('renders rate-limit failures in the custom error UI', async ({ page }, testInfo) => {
    ...
    await expect(page.getByRole('alert')).toContainText('Too many messages sent. Please try again later.');
    await expect(page.getByRole('button', { name: 'Send Message' })).toBeEnabled();
    await expectNoHorizontalOverflow(page);
  });
  ...
  test('opens a themed custom topic dropdown with keyboard support', async ({ page }, testInfo) => {
    ...
    const listboxBackground = await page.locator('#contact-topic-listbox').evaluate((element) => getComputedStyle(element).backgroundColor);
  ...
  test('keeps the message textarea scrollbar and resize grip usable', async ({ page }, testInfo) => {
    const assertNoConsoleErrors = installConsoleErrorGuard(page, testInfo);

    await page.setViewportSize({ width: 390, height: 844 });
    await page.goto('/contact');

    const longMessage = Array.from({ length: 24 }, (_, index) => `Line ${index + 1} with enough message text to create textarea overflow.`).join('\n');
    ...
    const textareaMetrics = await page.locator('#contact-message').evaluate((element) => {
      ...
      const gripRect = element.parentElement?.querySelector('.contact-textarea-grip')?.getBoundingClientRect();
    ...
    const messageHeightBefore = await page.locator('#contact-message').evaluate((element) => element.getBoundingClientRect().height);
    ...
    const messageHeightAfter = await page.locator('#contact-message').evaluate((element) => element.getBoundingClientRect().height);
