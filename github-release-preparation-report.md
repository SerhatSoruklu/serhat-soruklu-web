...
The regenerated ChatPDM and Coupyn PNGs were already in the original 59-file untracked allowlist; their two SVG sources and the renderer explain the OG-related count change. The approved SVG sources remain unchanged by the final correction. The checked-in renderer loads the SVGs' exact local Inter and Open Sans logo faces plus self-hosted Liberation Sans for Arial-compatible card typography, verifies that their wordmarks do not overlap, and performs a direct one-to-one Chromium rasterisation of each approved 1200 × 630 SVG without DOM or composition overrides. Each PNG therefore preserves its SVG source layout and premium dark black/gold/blue identity without host-dependent fallback-font changes.
